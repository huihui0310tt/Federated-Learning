...
def federated_learning():
    ...
    for round_idx in range(rounds):
        ...
        n_list = []
        best_accuracy = 0
        keep_global_model = copy.deepcopy(global_model)
        for i in clients:
            ...
            a_list.append(i.a_i)
            d_list.append(i.norm_grad)
            n_list.append(i.sample)

        print('---Global model---')
        metrics, global_model = aggregator.merge(keep_global_model, a_list, d_list, n_list, clients, model, dataset, batch_size, num_classes, no_cuda, gpu_devicename)
