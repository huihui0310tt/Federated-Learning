...
class Client:
    ...
    def train(self, epochs, global_model, lr, modelname, dataset, batch_size, num_classes, no_cuda, gpu_devicename):
        ...
        if global_model is None:
            global_model_para = None
        else:
            model.load_state_dict(global_model)
            global_model_para = copy.deepcopy(global_model)
        model.train()
        ...
        if global_model == None:
            norm_grad = {}
            for key in net_para:
                norm_grad[key] = torch.true_divide(net_para[key], self.a_i)
        else:
            norm_grad = global_model_para
            for key in norm_grad:
                norm_grad[key] = torch.true_divide(global_model_para[key]-net_para[key], self.a_i)
