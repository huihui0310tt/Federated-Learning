...
class Aggregator:
    ...
    def merge(self, keep_global_model, a_list, d_list, n_list, clients, modelname, dataset, batch_size, num_classes, no_cuda, gpu_devicename):
        ...
        if keep_global_model == None: 
            merged = {}
            for key in weights[0].keys():
                merged[key] = sum([w[key] * f for w, f in zip(weights, factors)])
        else: 
            total_n = sum(n_list)
            d_total_round = copy.deepcopy(keep_global_model)
            ...
            updated_model = keep_global_model
            for key in updated_model:
                updated_model[key] -= coeff * d_total_round[key]
            # global_model.load_state_dict(updated_model)

            merged = updated_model
